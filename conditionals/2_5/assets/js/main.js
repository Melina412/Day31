...
function check() {
  ...
  const eingabe = inputElement.value; // variable für eingegebenes bundesland

  const datensatz = dataBundeslaender.find(
    (item) => item.Bundesland === eingabe // durchsucht items im array nach bundesl. & vergleicht mit eingabe
  );

  if (datensatz) {
    // bundesland gefunden -> info wird ins html geschrieben
...
// textContent ist nur text (kein html), also in dem fall ausreichend
